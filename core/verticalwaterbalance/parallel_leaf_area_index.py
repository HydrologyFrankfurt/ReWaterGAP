...
def daily_leaf_area_index(temperature, growth_status, days, initial_days,
    ...
    # Growing Phase
    # ======================
    # Plant can only start growing if daily temperature is
    # above 8◦C(281.15 K) for a land-cover-specific number of initial days (See
        ...
        if growth_status == 0:
            if days >= initial_days:
                ...
                if cum_precipitation > 40:
                    ...
                    # Landcover could not grow but temperature specification is
                    # reached therefore LAI is minimum. The days variable is
                    # set to landcover specific initial days till cumulative
                    # precipitation is reached (thus growing season is reached)
            ...
            if days <= 30:
                days = days-1
                # if land_cover_type is '1' or '2' we have ervergreen plants
                # and LAI will never completely degrade and hence growth status
                # is switched at once.
                if land_cover <= 2:
                    growth_status = 0
                if days <= 0:
                ...
                if humid_arid == 1 and precipitation < 0.5:
                    ...
                    # when temperature is < 281.15 (see no growing phase lines
                    # 245-252) except for arid regions with precipitation
                    # below 0.5mm which decreases whether temperature condition
                    # is met or not.
    ...
    # ======================
        ...
        if growth_status == 0:
            if days > initial_days:
                ...
                if cum_precipitation > 40:
                    # Here temperature is less than  281.15K but landcover is
                    # already growing.This part of the code
            ...
            # Leaf Area index has reached maximum (growth status is 1 ) and
            # will deacrease when growing season conditions are not fulfilled
            # anymore. Here days variable decreases from 30 to 0 days
            ...
            if days <= 30:
                ...
                # constant (at maximum) for the stated land-cover-specific
                # number of days. Here the days variable decreases (note the
                # difference from the growing pahse)
