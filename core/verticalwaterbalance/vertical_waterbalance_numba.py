...
from core.verticalwaterbalance import compute_radiation_evapotranspiration as rad_pet
from core.verticalwaterbalance import numba_leaf_area_index as numba_lai
from core.verticalwaterbalance import canopy
from core.verticalwaterbalance import numba_snow_subgrid as snow
from core.verticalwaterbalance import numba_soil as soil
...
def vert_water_balance(rout_order, temperature, down_shortwave_radiation,
    ...
    #                  ||    Radiation and PET        ||
    #                  =================================
    net_radiation = np.zeros(temperature.shape)
    openwater_net_radiation = np.zeros(temperature.shape)
    daily_potential_evap = np.zeros(temperature.shape)
    openwater_potential_evap = np.zeros(temperature.shape)

    #                  =================================
    #                  ||    Leaf area index          ||
    #                  =================================
    leaf_area_index = np.zeros(temperature.shape)

    #                  =================================
    #                  ||    Canopy (Intercrption)    ||
    #                  =================================
    canopy_storage_out = np.zeros(temperature.shape)
    throughfall = np.zeros(temperature.shape)
    canopy_evap = np.zeros(temperature.shape)
    pet_to_soil = np.zeros(temperature.shape)
    land_storage_change_sum = np.zeros(temperature.shape)

    #                  =================================
    #                  ||           Snow              ||
    #                  =================================
    snow_water_storage_out = np.zeros(temperature.shape)
    snow_water_storage_subgrid_out = np.zeros(elevation[1:].shape)
    snow_fall = np.zeros(temperature.shape)
    sublimation = np.zeros(temperature.shape)
    snow_melt = np.zeros(temperature.shape)
    effective_precipitation = np.zeros(temperature.shape)
    max_temp_elev = np.zeros(temperature.shape)

    #                  =================================
    #                  ||           Soil             ||
    #                  =================================
    soil_water_content_out = np.zeros(temperature.shape)
    immediate_runoff = np.zeros(temperature.shape)
    groundwater_recharge_from_soil_mm = np.zeros(temperature.shape)
    surface_runoff = np.zeros(temperature.shape)
    ...
    for routflow_looper in range(len(rout_order)):
        ...
        # =================================================================
        daily_leaf_area_index = numba_lai.\
