...
class RadiationPotentialEvap:
    ...
    def __init__(self, climate_forcing, static_data, date, snow_water_storage):
        ...
        parameters = self.static_data.canopy_model_parameters
        ...
        for i in range(len(parameters)):
            albedo[land_cover[:, :] == parameters.loc[i, 'Number']] = \
                parameters.loc[i, 'albedo']

        snow_albedo = np.zeros(temperature.shape)
        snow_albedo.fill(np.nan)
        for i in range(len(parameters)):
            snow_albedo[land_cover[:, :] == parameters.loc[i, 'Number']] = \
                parameters.loc[i, 'snow_albedo']

        #  snow_water_storage > 3mm, snow abledo is used for shortwave
        # radiation calulation
        snow_albedo_thresh = pm.snow_albedo_thresh
        ...
        for i in range(len(parameters)):
            emissivity[land_cover[:, :] == parameters.loc[i, 'Number']] = \
                parameters.loc[i, 'emissivity']
        ...
        #print(self.net_shortwave_radiation[74, 187])
    ...
    def priestley_taylor(self):
        ...
        alpha_pt_coefficient = np.where(humid_arid == 1, pm.pt_coeff_arid,
                                        pm.pt_coeff_humid)
