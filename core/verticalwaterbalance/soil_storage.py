...
class SoilStorage:
    ...
    static_data : contains the following data
        ...
        drainage_direction : array
            Drainage direction taken based on [1]_, Units: (-)
    ...
    def __init__(self, static_data):
        ...
        for i in range(len(parameters)):
            rooting_depth[land_cover[:, :] == parameters.loc[i, 'Number']] = \
                parameters.loc[i, 'rooting_depth']
    ...
    def immediate_runoff(self, effective_precipitation):
        ...
        immediate_runoff = \
            np.where(self.builtup_area > 0,
                     (pm.runoff_frac_builtup * effective_precipitation
                      * self.builtup_area), 0)
    ...
    def daily_soil_storage(self, soil_water_content, pet_to_soil,
        ...
        runoff = effective_precipitation * (soil_saturation)**pm.gamma
...
# =============================================================================
        ...
        # ==============================================================
        runoff *= pm.areal_corr_factor
        immediate_runoff *= pm.areal_corr_factor
        ...
        groundwater_recharge_from_soil_mm_arid = \
            np.where(precipitation <= pm.critcal_gw_precipitation, 0,
                     groundwater_recharge_from_soil_mm)
        ...
        potential_gw_recharge = \
            np.where(precipitation <= pm.critcal_gw_precipitation,
        ...
        soil_water_content_new += (potential_gw_recharge/pm.areal_corr_factor)
        ...
        soil_water_overflow_new = \
            np.where((self.max_soil_water_content > 0) &
                     ...
                     (soil_water_overflow + soil_water_content_new -
                      self.max_soil_water_content) * pm.areal_corr_factor, 0)
        ...
        total_daily_runoff = \
            np.where((max_temp_elev > snow_freeze_temp), total_daily_runoff,
                     (soil_water_overflow) * pm.areal_corr_factor)
        ...
        corr_land_aet =\
            land_storage_change_sum * (pm.areal_corr_factor - 1.0) - \
            precipitation * (pm.areal_corr_factor - 1.0) + \
            (actual_soil_evap + canopy_evap + sublimation) * \
            pm.areal_corr_factor
