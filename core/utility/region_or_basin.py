...
class Select_region:
    ...
    def __init__(self, run_region,  arc_id,  stations, lat_lon_arcid, inflow_cell):
        self.region = 0
        if run_region is True: 
            ...
            all_upstream = Select_region.get_all_upstream_cells_arcid(selected_arcids, inflow_cell, upstream_cells)

            # create a 2d mask for the selected basin 
            mask = arc_id.isin(all_upstream)
            self.region = np.where(mask==True, 0,np.nan) 

        else:
            self.region = arc_id * 0
    ...
    def get_all_upstream_cells_arcid(arcid_list, inflow_cell, upstream_cells):
        ...
        if len(temp_upstream_cell) != 0 :
            Select_region.get_all_upstream_cells_arcid(temp_upstream_cell, inflow_cell, upstream_cells)
